...
namespace TRIM_REDUNT
{
    ...
    void Trim_redunt::make_page_content_index(const std::string& index_path)
    {
        ...
        std::ofstream fout(conf.config[index_path].c_str());
        std::string line;
    ...
    //docid title_offset title_len abstract_offset abstract_len content_offset content_len
    void Trim_redunt::make_page_index(const std::string& index_path)//读取去重后的新网页库，生成网页索引存入磁盘
    {
        std::ifstream fin(conf.config["new_page_lib"].c_str());
    ...
    void Trim_redunt::make_page_title_abstract()//制作标题，摘要,写入p_ta_index
    {
        std::ifstream fsrc(conf.config["new_page_lib"].c_str());
        ...
        for(iter;iter!=p_c_index.end();iter++)//遍历存放索引文件的hash_map，获取文档内容
        {
            ...
            std::vector<std::string> word; //存放文档内容分词
            seg.cut(content,word);         //对提取的文档内容分词
            ...
            std::priority_queue<CMypair> topK;
            ...
            while(topK.size())//将优先级队列中的元素弹出放入p_w_index 
            {
                std::pair<std::string,int> w_pair=make_pair(topK.top().m_word,topK.top().m_cnt);
                word_freq_vec.push_back(topK.top().m_word);
                p_w_index[iter->first].insert(w_pair);
                topK.pop();
            }
            std::string title;
            std::string abstract;
            for(int index=0;index!=word_freq_vec.size();index++)//PRIORITY_NUM个key制作摘要，词频最大的PRIORITY_NUM-15个key制作标题
            {
    ...
    bool Trim_redunt::doc_redunt(keyword& left,keyword& right) const//根据两篇文档词频最高的PRIORITYN_NUM个key，判断两篇文档是否重复
    {
    ...
    void Trim_redunt::remv_dup(const std::string& page_lib_path)//冒泡去重，去重后重写网页库，该网页库含有标题、摘要
    {
    ...
    void Trim_redunt::rwrite_page_lib(const std::string& page_lib_path,const std::string& title,const std::string& abstract,const std::string& context,const int& doc_id)
    {
        ...
        std::ofstream fout;
        fout.open(conf.config[page_lib_path].c_str(),std::ofstream::out | std::ofstream::app); 
        fout<<"<doc>"<<std::endl;
    ...
    void Trim_redunt::make_page(const char* src_path,const char* des_path)//将文档内容提取出来并且转码
    {
        ...
        std::ofstream fout;
        fout.open(conf.config[des_path].c_str(),std::ofstream::out | std::ofstream::app); 
        std::string line;
        std::string context;
        context.clear();
        int flag=0;
    ...
    void Trim_redunt::make_page_lib(const char* dir_path,const char* des_path)
    {
