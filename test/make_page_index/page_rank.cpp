...
namespace PAGE_RANK
{
    ...
    //倒排索引格式：key1 docid11 相对权重11 docid12 相对权重12 docid13 相对权重13 ...
    //遍历倒排索引，获取文档内容 -> 分词 -> 计算TF,存入word_tf_idf中 -> 替换word_tf_idf中TF为权重，再替换为相对权重 -> 写入磁盘
    void Page_rank::make_word_page_rindex(const std::string& word_page_rindex_path)
    {
        ...
        //遍历存放索引文件的hash_map，即遍历每一篇文档，获取文档内容 -> 分词 -> 计算TF,存入word_tf_idf中
        for(iter;iter!=page_index.end();iter++)
        {
            total_doc++; //总文档数目
            std::cout<<"doc_id"<<iter->first<<"'s task"<<std::endl;
            keyword doc_word_freq; //存放一个文档的词汇,词 词频
            fsrc.seekg((iter->second)[2].first); //将文件流指针定位文档内容
            ...
            for(iter1;iter1!=word.end();iter1++) //统计每个key在一篇文档中的频率，将其放入doc_word_freq中
            {
            ...
            for(word_iter;word_iter!=doc_word_freq.end();word_iter++)//计算每个key在每篇文档中的TF
            {
                //double doc_TF=(double)(word_iter->second)/(double)word.size();词频/总词数
        ...
        for(index_iter;index_iter!=word_tf_idf.end();index_iter++)//遍历词汇倒排索引，计算每个key出现在文档中的数目,将doc_TF替换成权重
        {
            word_IDF=log((double)total_doc/((double)word_tf_idf[index_iter->first].size()+1));//IDF=log(文档总数/(包含该词的文档数+1))
            std::map<int,double>::iterator iter=(index_iter->second).begin();
            for(iter;iter!=(index_iter->second).end();iter++)//计算TF*IDF，存入word_tf_idf中
            {
                double weight=(iter->second)*word_IDF;	//TF*IDF
        ...
        for(index_iter;index_iter!=word_tf_idf.end();index_iter++)//计算word_tf_idf中word在每个文档的相对权重
        {
            ...
            {
                double r_weight=(iter->second)/(doc_weight_index[iter->first]);//相对权重 = 权重/一篇文档的总权重
    ...
    //格式： docid1 key11 weight11 key12 weight12 key13 weight13 ...
    void Page_rank::make_page_weight_rindex(const std::string& p_w_rindex_path)
    {
        ...
        int id_num=0;
        for(iter;iter!=word_tf_idf.end();iter++)//遍历词汇倒排索引，建立docid倒排索引
        {
            std::map<int,double>::iterator iter1=(iter->second).begin();
            for(iter1;iter1!=(iter->second).end();iter1++)
            {
                std::pair<std::string,double> apair=make_pair(iter->first,iter1->second);//key weight
        ...
        for(p_w_riter;p_w_riter!=p_w_rindex.end();p_w_riter++)//写入磁盘
        {
    ...
    void Page_rank::make_page_module_index(const std::string& page_module_path)
    {
        ...
        {
            std::map<std::string,double>::iterator iter1=(p_w_riter->second).begin();
            double squ_value=0;//一篇文档中相对权重weight的平方和
            for(iter1;iter1!=(p_w_riter->second).end();iter1++)
            {
    ...
    void Page_rank::get_doc_intersec()//保存文档的一些docid，这些docid文档中包含所有查询词
    {
        page_weight_rindex_iter p_w_riter=p_w_rindex.begin();
        for(p_w_riter;p_w_riter!=p_w_rindex.end();p_w_riter++)//取doc_id_set初值，获取网页倒排索引中包含的全部docid
        {
            doc_id_set.insert(p_w_riter->first);
        }
        tf_iter query_iter=query_tf_idf.begin();
        //每遍历一个查询词，就缩小doc_id_set范围，遍历到最后，doc_id_set中的docid文档包含所有查询词
        for(query_iter;query_iter!=query_tf_idf.end();query_iter++)//遍历查询词的(keyi weighti)
        {
            std::set<int>::iterator s_iter=doc_id_set.begin();
            std::set<int> temp_set;
            for(s_iter;s_iter!=doc_id_set.end();s_iter++)//根据docid遍历网页权重倒排索引p_w_rindex
            {
                if(p_w_rindex[*s_iter].count(query_iter->first)>0)//如果一篇文章包含查询key，保存该docid
                {
    ...
    void Page_rank::get_doc_similar()//根据余弦相似度求文本相似度
    {
        std::set<int>::iterator iter=doc_id_set.begin();
        for(iter;iter!=doc_id_set.end();iter++)//遍历含有所有查询词的docid
        {
            tf_iter query_iter=query_tf_idf.begin();
            double cross_product=0;
            for(query_iter;query_iter!=query_tf_idf.end();query_iter++)//两向量对应元素乘积和
            {
                cross_product+=(p_w_rindex[*iter][query_iter->first])*(query_iter->second);
            }
            double multi_module=query_module*(page_module[*iter]);//两向量模的积
            double doc_similar=cross_product/multi_module;//文本相似度
